...
app.get("/public-tutors/:id", async (req, res) => {
  try {
    ...
    const tutor = await User.findOne({ _id: id, role: "tutor" }).select(
      "-password -__v -studentData"
    );

    if (!tutor) {
      return res
        .status(404)
        .send({ message: "Tutor not found or is not active" });
    }
...
app.get("/tutor/analytics", verifyToken, async (req, res) => {
  try {
    ...
    if (req.user.role !== "tutor") {
      return res
        .status(403)
        .send({ message: "Forbidden: Only tutors can access this." });
    }

    const applications = await JobApplication.find({
      tutorId: tutorId,
    }).populate("postId", "subject");

    const sessions = await TuitionSession.find({ tutorId: tutorId }).populate(
      "postId",
      "subject"
    );
...
app.get("/student/stats", verifyToken, async (req, res) => {
  try {
    ...
    if (req.user.role !== "student") {
      return res
        .status(403)
        .send({ message: "Forbidden: Only students can access this." });
    }
    ...
    const statusData = [
      ...
      {
        name: "Active/Booked",
        value: statusCounts.booked + statusCounts.approved,
      },
    ...
    res.send({
      ...
      avgBudget:
        totalPosts > 0
          ? Math.round(posts.reduce((s, p) => s + p.budget, 0) / totalPosts)
          : 0,
      budgetData,
      statusData,
    });
...
app.patch("/sessions/:id/complete", verifyToken, async (req, res) => {
  try {
    ...
    res.send({
      success: true,
      message: "Session completed! Funds released to tutor.",
    });
  } catch (error) {
